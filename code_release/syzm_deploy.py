...
def codeupdate(number):
    ...
    try:
        ...
        code_update = Popen(SVN_UPDATE_COMMAND, shell=True, stdout=PIPE, stderr=PIPE)
        while True:
            # 编译显示信息
...
def codewar(version, project_name, code_time):
    if codebuild() == 0:
        ...
        # 项目包生成路径
        PROJECT_WAR_PATH = "%s/cybershop-%s/target/%s" % (export_path, project_name, CODE_WAR_NAME) + ".war"
        # 直接将项目包解压到项目目录
        try:
...
def rollbackdeploy(svn_number, project_name, tags_name):
    for number in checkdeploy():
        ...
        if number == svn_number and project_name == pro_name:
            ...
            ansible_path = "ansible-playbook"
            other_vars = "hosts=%s project_name=%s repository=%s time=%s svn_number=%s" % (project_name, project_name, PROJECT_REPOSITORY, code_time, svn_number)
            playbook_path = "/etc/ansible/roles/syzm.yml"
...
def main():
    ...
    if args.operate == 'rollback':
        rollbackdeploy(svn_number=args.number, project_name=args.project, tags_name=args.operate)
